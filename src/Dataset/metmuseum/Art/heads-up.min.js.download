;(function(window,document,undefined){console.log("==headsUp==");'use strict';function extend(a,b){for(var key in b){if(b.hasOwnProperty(key)){a[key]=b[key];}}
return a;}
...
if(newScrollY>20){addClass(selector,'sticky-header');}else{removeClass(selector,'sticky-header');}
oldScrollY=newScrollY;}
if(selector){resizeHandler();window.addEventListener('resize',throttle(resizeHandler),false);window.addEventListener('scroll',throttle(scrollHandler,100),false);}}};window.headsUp=headsUp;})(window,document);new headsUp('.page-header');