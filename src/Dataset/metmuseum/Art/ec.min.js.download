...
proto.CONST_KEY_PROMOTION='persist_promotion';return proto;})(),Checkout:(function(){return{data:{},tracked:{},step:function(previous,current,currentCode){if(this.data&&this.data.hasOwnProperty('ecommerce')){if(currentCode=='payment'){this.data.ecommerce.checkout.actionField.step=AEC.Const.CHECKOUT_STEP_PAYMENT;this.data.ecommerce.checkout.actionField.option='Billing';}else if(currentCode=='shipping'){this.data.ecommerce.checkout.actionField.step=AEC.Const.CHECKOUT_STEP_SHIPPING;this.data.ecommerce.checkout.actionField.option='Shipping';}else{this.data.ecommerce.checkout.actionField.step=++current;}
if(AEC.Const.COOKIE_DIRECTIVE){if(AEC.Const.COOKIE_DIRECTIVE_CONSENT_GRANTED){dataLayer.push(this.data);}}
