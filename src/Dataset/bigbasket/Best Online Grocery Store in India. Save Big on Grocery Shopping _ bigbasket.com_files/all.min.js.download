  ...
  controller: [
    ...
    function($scope, $http, $window, loginAndSignupService, $timeout) {
      ...
      vm.facebookLogin = function() {
        // Snowplow = Social login clicked - Facebook
        vm.bbLoader = true;
        var data = JSON.stringify({
          EventName: "Social_Clicked",
          EventSubGroup: "login",
      ...
      var initLoginWithOTPForm = function(identifier) {
        ...
        vm.loginWithOTP.sendOTP = function() {
          ...
          loginAndSignupService
            .postHulk(
              "/mapi/v4.0.0/member-svc/otp/send/",
              otpRequestInput,
              parent
            )
            .then(function(response) {
            ...
            .catch(function(err) {
              vm.bbLoader = false;
              vm.showResend = false;
              vm.otpResendMessage = "Something went wrong,Please try again";
              $timeout(function() {
                vm.otpResendMessage = "";
              }, 6000);
            });
              ...
              if (response.success) {
                if (!response.error_code) {
                  ...
                  if (
                    ...
                    $(document).trigger("loginInteractions", [snowplowData]);
                    vm.loginWithOTP.serverError = response.message;
...
angular.module("app.layout").controller("loginAndSignupBase", [
  ...
  function($scope, $uibModalInstance, modalData, loginAndSignupService) {
    ...
    $scope.dismiss = function() {
      if (window.loginScreen === "almostThere") {
    ...
    function UserDataservice($http, $q,toastr,$rootScope) {
        ...
        function updateCartQuantity(productId,quantity,sid){
            ...
            function updateCartComplete(response){
                ...
                if(!added){
                    toastr.error('',response.data.mesg,{timeOut: 500,positionClass: "toast-top-full-width",closeButton: true,preventDuplicates: true,preventOpenDuplicates: true});
                }
...
$(document).ready(function () {
    ...
    if (newLogin || authModal || loginUrl || signupUrl) {
        ...
        setTimeout(function () {
            if (mid) {
                document.getElementById("kapchat-widget-btn").click();
            }
            else {
                // window.openLoginModal();
            }
